...
import { Plane, CreditCard, ShoppingBag, LineChart } from 'lucide-react'
...
export function PublishersHeroPanel() {
  const { t } = useTranslations()
  const [v0, v1, v2] = HOME_CREATOR_CASE_VIDEO_SRCS
      ...
      <div className="mx-auto max-w-[min(100rem,calc(100vw-2rem))] overflow-hidden rounded-[2.5rem] bg-[#f5f5f5] lg:rounded-[3rem]">
        <div className="flex flex-col gap-10 px-5 py-10 pl-6 pr-5 sm:px-8 sm:py-12 lg:flex-row lg:items-center lg:justify-between lg:gap-6 lg:pl-14 lg:pr-0 lg:pt-14 lg:pb-10">
          <div className="max-w-xl shrink-0 space-y-8 lg:max-w-[26rem] xl:max-w-xl">
            <h1
              className="font-[family-name:var(--font-heading)] text-[clamp(2.25rem,4.5vw,4.5rem)] font-bold leading-[1.15] tracking-tight"
              style={{ color: NAVY }}
            >
              <span style={{ color: ACCENT }} className="block">
                {t('publishers.pageHeroAccent')}
              </span>
              <span className="block">{t('publishers.pageHeroLine2')}</span>
              <span className="block">{t('publishers.pageHeroLine3')}</span>
              <span className="block">{t('publishers.pageHeroLine4')}</span>
            </h1>
            <Link
              href="/get-started"
                ...
                <div className="grid w-full max-w-[28rem] grid-cols-2 gap-2 sm:max-w-[32rem] sm:gap-3 lg:max-w-[36rem]">
                  <div className="flex flex-col gap-2 sm:gap-3">
                    <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-black/5">
                      <CollageVideo src={v0} className="absolute inset-0 h-full w-full object-cover" />
                    </div>
                    <div className="flex items-center gap-2 rounded-2xl bg-[#bef7be]/90 px-3 py-2.5 backdrop-blur-sm ring-1 ring-black/5 sm:px-4 sm:py-3">
                      <Plane className="size-5 shrink-0 text-[#262626] sm:size-6" strokeWidth={1.75} aria-hidden />
                      <span className="font-[family-name:var(--font-heading)] text-base font-medium capitalize text-[#262626] sm:text-lg">
                        {t('publishers.catTravel')}
                      </span>
                    </div>
                    <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-black/5">
                      <Image
                    ...
                    <div className="flex items-start justify-between gap-2 rounded-2xl bg-[#ffd8e3]/95 px-3 py-3 backdrop-blur-sm ring-1 ring-black/5 sm:px-4">
                      <span className="font-[family-name:var(--font-heading)] text-base font-medium capitalize leading-snug text-[#262626] sm:text-lg">
                        {t('publishers.catOnlineServices')}
                      </span>
                      <CreditCard className="size-6 shrink-0 text-[#262626] opacity-80 sm:size-7" strokeWidth={1.75} aria-hidden />
                    </div>
                  </div>

                  <div className="flex flex-col gap-2 sm:gap-3">
                    <div className="flex items-center gap-2 rounded-2xl bg-[#bcd2ff] px-3 py-2.5 ring-1 ring-black/5 sm:px-4 sm:py-3">
                      <ShoppingBag className="size-5 shrink-0 text-[#262626] sm:size-6" strokeWidth={1.75} aria-hidden />
                      <span className="font-[family-name:var(--font-heading)] text-base font-medium capitalize text-[#262626] sm:text-lg">
                        {t('publishers.catRetail')}
                      </span>
                    </div>
                    <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-black/5">
                      <CollageVideo src={v1} className="absolute inset-0 h-full w-full object-cover" />
                    </div>
                    <div className="flex items-center gap-2 rounded-2xl bg-[#fff7a0] px-3 py-2.5 ring-1 ring-black/5 sm:px-4 sm:py-3">
                      <LineChart className="size-5 shrink-0 text-[#262626] sm:size-6" strokeWidth={1.75} aria-hidden />
                      <span className="font-[family-name:var(--font-heading)] text-base font-medium capitalize text-[#262626] sm:text-lg">
                        {t('publishers.catFintech')}
                      </span>
                    </div>
                    <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-black/5">
                      <CollageVideo src={v2} className="absolute inset-0 h-full w-full object-cover" />
                    </div>
                  </div>
                </div>
              </div>
