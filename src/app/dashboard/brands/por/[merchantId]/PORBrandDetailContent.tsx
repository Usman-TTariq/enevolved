...
type Merchant = {
  merchant_id: string; name: string; url: string | null; tracking_url: string | null;
  logo_url: string | null; category: string | null; description: string | null;
  commission_rate: string | null; average_commission: string | null;
  average_basket: string | null; cookie_length: number | null;
  deep_links: boolean; merchant_status: string; affiliate_status: string | null;
  conversion_ratio: string | null; approval_rate: string | null; void_rate: string | null;
};
...
export default function PORBrandDetailContent({ merchantId }: { merchantId: string }) {
  ...
  const load = useCallback(async (showSpinner = true) => {
    ...
    const res = await fetch(`/api/publisher/por/brands/${merchantId}`, { credentials: "include" });
  ...
  const applyNow = async () => {
    ...
    setApplying(true);
    try {
      const res = await fetch("/api/publisher/por/apply", {
  ...
  const createLink = async () => {
    ...
    try {
      const res = await fetch("/api/publisher/por/go-links", {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ merchantId: merchant.merchant_id, landingPage: landingPage.trim() || undefined }),
      });
      const data = await res.json();
        ...
        <aside className="w-full lg:w-56 lg:shrink-0 space-y-4">
          <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
            ...
            <div className="flex flex-col items-center p-5 text-center">
              ...
              <p className="mt-0.5 text-[11px] text-gray-400">PaidOnResults · 🇬🇧 UK</p>

              {merchant.url && (
                <a href={merchant.url} target="_blank" rel="noreferrer"
                  className="mt-2 flex items-center gap-1 rounded-xl border border-gray-200 px-3 py-1.5 text-xs font-semibold text-orange-600 hover:border-orange-300 hover:bg-orange-50 transition-colors">
                  Visit store
            ...
            <div className="border-t border-gray-50 divide-y divide-gray-50">
              {[
                ...
                ["Avg basket",   merchant.average_basket ?? "—"],
                ["Cookie",       merchant.cookie_length ? `${merchant.cookie_length} days` : "—"],
                ["Deep linking", merchant.deep_links ? "Allowed" : "Not supported"],
          ...
          {merchant.description && (
            <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
              ...
              <p className="text-xs leading-relaxed text-gray-600 line-clamp-6">{merchant.description}</p>
            </div>
          )}
        </aside>
        ...
        <div className="flex-1 min-w-0 space-y-4">
            ...
            {TABS.map((t) => (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`pb-2.5 px-1 mr-4 text-sm font-semibold transition-all ${
                  tab === t.id ? "border-b-2 border-orange-500 text-orange-700" : "text-gray-400 hover:text-gray-700"
                }`}>
                {t.label}
              </button>
            ))}
            ...
            <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
              ...
              <div className="divide-y divide-gray-50">
                {[
                  ...
                  ["Category",       merchant.category ?? "—"],
                  ["Cookie window",  merchant.cookie_length ? `${merchant.cookie_length} days` : "—"],
                  ["Deep linking",   merchant.deep_links ? "Supported" : "Not supported"],
                  ["Status",         merchant.merchant_status ?? "LIVE"],
                  ["Conversion rate", merchant.conversion_ratio ?? "—"],
                  ["Approval rate",  merchant.approval_rate ?? "—"],
                ].map(([label, value]) => (
          ...
          {tab === "commission" && (
            <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
              ...
              <div className="divide-y divide-gray-50">
                {[
                  ["Commission rate",  merchant.commission_rate ?? "—"],
                  ["Average commission", merchant.average_commission ?? "—"],
                  ["Average basket",   merchant.average_basket ?? "—"],
                  ["Currency",         "GBP"],
                  ["Void rate",        merchant.void_rate ?? "—"],
                ].map(([label, value]) => (
                  <div key={label} className="flex items-baseline gap-4 px-5 py-3">
                    <span className="min-w-[160px] text-sm text-gray-500 shrink-0">{label}</span>
                    <span className={`text-sm font-${label === "Commission rate" ? "bold text-emerald-700" : "normal text-gray-800"}`}>{value}</span>
                  </div>
                ))}
              </div>
              <div className="bg-orange-50 border-t border-orange-100 px-5 py-3">
                <p className="text-xs text-orange-700">
                  Commission is paid in GBP on validated orders. PaidOnResults validates transactions before payout.
                </p>
              </div>
            </div>
          )}
                ...
                <>
                  <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
                    <div className="border-b border-gray-50 px-5 py-4">
                      ...
                      <p className="mt-0.5 text-xs text-gray-400">Promote this merchant with a short Earnytics URL.</p>
                    </div>
                    <div className="p-5 space-y-4">
                      <div>
                        <label className="mb-1.5 block text-[10px] font-black uppercase tracking-widest text-gray-400">Campaign</label>
                        <div className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm text-gray-700">{merchant.name}</div>
                      </div>
                      {merchant.deep_links && (
                        <div>
                          <label className="mb-1.5 block text-[10px] font-black uppercase tracking-widest text-gray-400">Landing page (optional)</label>
                          <input value={landingPage} onChange={(e) => setLandingPage(e.target.value)}
                            placeholder={`e.g. https://${(merchant.url ?? "").replace(/^https?:\/\//, "").split("/")[0]}/sale`}
                            className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:border-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-100" />
                          <p className="mt-1 text-[11px] text-gray-400">Your go-link slug is used as the custom tracking ID for attribution.</p>
                        </div>
                      )}
                      ...
                      {lastCreated && (
                        <div className="flex items-center gap-2 rounded-xl border border-orange-200 bg-orange-50 px-4 py-3">
                          <code className="flex-1 truncate text-sm font-medium text-orange-800">{lastCreated}</code>
                  ...
                  {goLinks.length > 0 && (
                    <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
                      <div className="border-b border-gray-50 px-5 py-3 flex items-center justify-between">
                        ...
                        <span className="rounded-full bg-orange-50 px-2.5 py-0.5 text-[11px] font-bold text-orange-700 border border-orange-100">{goLinks.length}</span>
                      </div>
                      <div className="divide-y divide-gray-50">
                        {goLinks.map((l) => {
                          const shortUrl = `${appUrl}/go/short/${l.slug}`;
                          return (
                            <div key={l.id} className="flex items-center gap-3 px-5 py-3 hover:bg-orange-50/30 transition-colors">
                              <div className="flex-1 min-w-0">
                                <code className="block truncate text-sm font-medium text-orange-700">{shortUrl}</code>
                              ...
                              <button onClick={() => void copy(shortUrl, l.id)}
                                className="shrink-0 rounded-xl border border-gray-200 bg-white px-3 py-1.5 text-xs font-semibold text-gray-600 hover:border-orange-300 hover:text-orange-700 transition-colors">
                ...
                <div className="flex flex-col items-center gap-4 rounded-2xl border border-gray-100 bg-white py-16 text-center shadow-sm">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-400">
                      {applicationStatus === "pending" ? "Admin will approve your application shortly." : "Apply to join this merchant first."}
                    </p>
                  </div>
                  {!applicationStatus && (
                    <button onClick={applyNow} disabled={applying}
