...
export async function POST(request: Request) {
  ...
  if (ids.length === 0) return NextResponse.json({ error: "campaignIds is required" }, { status: 400 });
  if (ids.length > 200) return NextResponse.json({ error: "Max 200 campaigns per request" }, { status: 400 });
  ...
  if (toInsert.length > 0) {
    // Verify campaign IDs exist in tradetracker_campaigns
    const { data: validCampaigns } = await supabase
      .from("tradetracker_campaigns")
      .select("tt_campaign_id")
      .in("tt_campaign_id", toInsert);

    const validIds = new Set((validCampaigns ?? []).map((c) => String(c.tt_campaign_id)));
    const rows = toInsert
    ...
    failed += toInsert.length - rows.length;
